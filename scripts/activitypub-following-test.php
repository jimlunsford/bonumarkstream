...
bms_ap_following_assert(preg_match('/<div class="following-actions stream-card-actions">.*?<\/div>\s*<\/div>\s*<\?php if \(!\$deleted\): \?>\s*<form method="post" class="following-reply-form comment-form">/s', $template) === 1, 'The remote reply form is still nested inside the horizontal interaction row.');

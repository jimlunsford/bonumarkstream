...
<section class="panel page-intro-panel theme-manager-intro">
  ...
  <p class="meta">Install, inspect, activate, configure, and delete code-free presentation themes.</p>
</section>
...
<section class="theme-card-grid theme-package-grid theme-manager-grid">
  <?php foreach ($packages as $key => $theme): ?>
    <?php
      ...
      $summary = function_exists('bms_public_theme_manager_summary') ? bms_public_theme_manager_summary($theme) : ['valid' => !empty($health['valid']), 'status_class' => !empty($health['valid']) ? 'ready' : 'needs-attention', 'asset_total' => 0, 'asset_missing' => 0, 'setting_total' => count((array)($theme['settings'] ?? [])), 'support_total' => 0];
    ...
    <article class="panel theme-card theme-package-card theme-manager-card <?= $isActive ? 'active-theme' : '' ?> <?= !$isValidTheme ? 'invalid-theme' : '' ?> status-<?= htmlspecialchars($statusClass, ENT_QUOTES, 'UTF-8') ?>">
      ...
      <div class="theme-card-body">
        ...
        <p class="theme-card-summary meta">
          <?= (int)($summary['asset_total'] ?? 0) ?> assets · <?= (int)($summary['setting_total'] ?? 0) ?> settings · <?= (int)($summary['support_total'] ?? 0) ?> capabilities
      ...
      <div class="theme-card-actions theme-manager-actions">
        <a class="button-link secondary" href="<?= htmlspecialchars(bms_admin_url('theme-details.php?slug=' . rawurlencode($slug)), ENT_QUOTES, 'UTF-8') ?>">Details</a>
        ...
        <?php else: ?>
          <a class="button-link secondary" href="<?= htmlspecialchars(bms_admin_url('theme-settings.php'), ENT_QUOTES, 'UTF-8') ?>">Settings</a>
        <?php endif; ?>

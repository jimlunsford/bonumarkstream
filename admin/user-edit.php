...
$user = $userId > 0 ? mp_find_user_by_id_any($userId) : null;
if (!$user) {
    mp_flash('User was not found.', 'error');
    mp_redirect(mp_admin_url('users.php'));
}

$currentUser = mp_current_user();
$currentUserId = (int)($currentUser['id'] ?? 0);
$isCurrentUser = (int)($user['id'] ?? 0) === $currentUserId;
$reassignTargets = mp_user_delete_reassign_targets((int)($user['id'] ?? 0));
$defaultReassignId = 0;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if ($action === 'update_account') {
            $user = mp_admin_update_user_account(
                $userId,
                (string)($_POST['username'] ?? ''),
                (string)($_POST['display_name'] ?? ''),
                (string)($_POST['email'] ?? ''),
                (string)($_POST['role'] ?? 'commenter'),
                (string)($_POST['status'] ?? 'active'),
                (string)($_POST['profile_visibility'] ?? 'public'),
                !empty($_POST['email_verified'])
            );
            mp_flash('User account updated.', 'success');
            mp_redirect(mp_admin_url('user-edit.php?id=' . $userId));
        }

        if ($action === 'reset_password') {
            mp_admin_reset_user_password(
                $userId,
                (string)($_POST['new_password'] ?? ''),
                (string)($_POST['confirm_password'] ?? '')
            );
            mp_flash('User password reset. Share the temporary password securely.', 'success');
            mp_redirect(mp_admin_url('user-edit.php?id=' . $userId . '#reset-password'));
        }

        if ($action === 'delete_user') {
            mp_admin_delete_user(
                $userId,
                (int)($_POST['reassign_to'] ?? 0),
                (string)($_POST['confirm_username'] ?? '')
            );
            mp_flash('User account deleted and owned records reassigned.', 'success');
            mp_redirect(mp_admin_url('users.php'));
        }

        mp_flash('Unknown user management action.', 'error');
        mp_redirect(mp_admin_url('user-edit.php?id=' . $userId));
    } catch (Throwable $e) {
...
function bms_admin_user_count_rows(int $userId): array
{
    ...
    try {
        $stmt = mp_db()->prepare('SELECT COUNT(*) FROM ' . mp_table('posts') . ' WHERE author_id = :id');
        $stmt->execute(['id' => $userId]);
        $counts['stream_posts'] = (int)$stmt->fetchColumn();
    } catch (Throwable $e) {}
    try {
        $stmt = mp_db()->prepare('SELECT COUNT(*) FROM ' . mp_table('comments') . ' WHERE user_id = :id');
        $stmt->execute(['id' => $userId]);
        $counts['comments'] = (int)$stmt->fetchColumn();
    } catch (Throwable $e) {}
    try {
        $stmt = mp_db()->prepare('SELECT COUNT(*) FROM ' . mp_table('media') . ' WHERE uploaded_by = :id');
        $stmt->execute(['id' => $userId]);
...
$viewProfileUrl = function_exists('mp_public_profile_url_for_user') ? mp_public_profile_url_for_user($user) : mp_url_path('profile.php?user=' . rawurlencode((string)($user['username'] ?? '')));

mp_admin_header('Edit User', [
    ['label' => 'All Users', 'href' => mp_admin_url('users.php'), 'style' => 'secondary'],
    ['label' => 'View Profile', 'href' => $viewProfileUrl, 'style' => 'secondary', 'target' => true],
]);
?>
<section class="panel page-intro-panel">
  <p class="eyebrow">User management</p>
  <h2><?= htmlspecialchars((string)($user['display_name'] ?? $user['username'] ?? 'User'), ENT_QUOTES, 'UTF-8') ?></h2>
  <p class="meta">Edit account details, reset the password, and manage account deletion from one place.</p>
</section>

<section class="panel">
  <div class="info-grid">
    <div class="info-card"><strong>Role</strong><p><?= htmlspecialchars(mp_role_label((string)($user['role'] ?? 'commenter')), ENT_QUOTES, 'UTF-8') ?></p></div>
    <div class="info-card"><strong>Status</strong><p><?= htmlspecialchars(mp_user_status_label((string)($user['status'] ?? 'active')), ENT_QUOTES, 'UTF-8') ?></p></div>
...
<section class="panel settings-panel">
  ...
  <form method="post" class="settings-form">
    ...
    <select id="role" name="role">
      <?php foreach (mp_roles() as $key => $label): ?>
        <option value="<?= htmlspecialchars($key, ENT_QUOTES, 'UTF-8') ?>" <?= mp_normalize_role((string)($user['role'] ?? 'commenter')) === $key ? 'selected' : '' ?>><?= htmlspecialchars($label, ENT_QUOTES, 'UTF-8') ?></option>
      <?php endforeach; ?>
    </select>

    <label for="status">Status</label>
    <select id="status" name="status">
      <?php foreach (mp_user_status_options() as $statusKey => $statusLabel): ?>
        <option value="<?= htmlspecialchars($statusKey, ENT_QUOTES, 'UTF-8') ?>" <?= mp_normalize_user_status((string)($user['status'] ?? 'active')) === $statusKey ? 'selected' : '' ?>><?= htmlspecialchars($statusLabel, ENT_QUOTES, 'UTF-8') ?></option>
      <?php endforeach; ?>
    </select>
    ...
    <div class="form-actions-row user-edit-actions">
      ...
      <a class="button secondary-button" href="<?= htmlspecialchars(mp_admin_url('users.php'), ENT_QUOTES, 'UTF-8') ?>">Cancel</a>
    </div>
  </form>
</section>
...
<section id="reset-password" class="panel settings-panel">
  ...
  <p class="meta">Set a temporary password for this user. They should change it after signing in.</p>
  <form method="post" class="settings-form">
    <input type="hidden" name="csrf_token" value="<?= htmlspecialchars(mp_csrf_token(), ENT_QUOTES, 'UTF-8') ?>">
...
<section class="panel danger-zone">
  ...
  <?php elseif (empty($reassignTargets)): ?>
    <p class="notice warning">Create or activate another account before deleting this user so owned records have somewhere safe to go.</p>
  <?php else: ?>
    ...
    <form method="post" class="settings-form" data-confirm="Delete this user account? This removes the login and reassigns owned records.">
      <input type="hidden" name="csrf_token" value="<?= htmlspecialchars(mp_csrf_token(), ENT_QUOTES, 'UTF-8') ?>">
      ...
      <select id="reassign_to" name="reassign_to" required>
        <?php foreach ($reassignTargets as $target): ?>
          ...
          <option value="<?= $targetId ?>" <?= $targetId === $defaultReassignId ? 'selected' : '' ?>><?= htmlspecialchars((string)($target['display_name'] ?? $target['username'] ?? 'User'), ENT_QUOTES, 'UTF-8') ?> (<?= htmlspecialchars(mp_role_label((string)($target['role'] ?? 'commenter')), ENT_QUOTES, 'UTF-8') ?>)</option>
        <?php endforeach; ?>
      </select>

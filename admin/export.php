...
function mp_export_add_directory(ZipArchive $zip, string $baseDir, string $zipPrefix = ''): void
{
    if (!is_dir($baseDir)) {
        return;
    }
...
function mp_export_markdown_from_database(ZipArchive $zip): int
{
    $count = 0;
    $manifest = [
        'generated_at' => date('c'),
    ...
    foreach ($sections as $section => $zipDir) {
        foreach (mp_list_content_records($section) as $page) {
            ...
            $filename = $slug . '.md';
            $raw = function_exists('mp_database_content_raw') ? mp_database_content_raw($page) : mp_build_markdown_document($page, (string)($page['body'] ?? ''));
...
function mp_export_database_tables(): array
{
    $prefix = mp_table_prefix();
    $like = str_replace(['\\', '%', '_'], ['\\\\', '\\%', '\\_'], $prefix) . '%';
    try {
        $stmt = mp_db()->query('SHOW TABLES LIKE ' . mp_db()->quote($like));
        ...
        return array_map('mp_table', $fallback);
    }
}

function mp_export_database_sql(): string
{
    $tables = mp_export_database_tables();
    ...
    foreach ($tables as $table) {
        ...
        if ($safeTable === '' || !str_starts_with($safeTable, mp_table_prefix())) {
            continue;
        }
        try {
            $rows = mp_db()->query('SELECT * FROM `' . $safeTable . '`')->fetchAll(PDO::FETCH_ASSOC) ?: [];
        } catch (Throwable $e) {
            continue;
        }
        $sql .= "-- Table: {$safeTable}\n";
        foreach ($rows as $row) {
            ...
            $values = array_map(function ($value) {
                ...
                return mp_db()->quote((string)$value);
            }, array_values($row));
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!class_exists('ZipArchive')) {
        mp_flash('Export requires PHP ZipArchive. Ask the host to enable it.', 'error');
        mp_redirect(mp_admin_url('export.php'));
    }
    $tmp = mp_root_path('tmp/exports');
    if (!is_dir($tmp)) { @mkdir($tmp, 0755, true); }
    $filename = 'bonumark-stream-export-' . $kind . '-' . date('Ymd-His') . '.zip';
    $path = $tmp . '/' . $filename;
    $zip = new ZipArchive();
    if ($zip->open($path, ZipArchive::CREATE | ZipArchive::OVERWRITE) !== true) {
        mp_flash('Could not create export ZIP.', 'error');
        mp_redirect(mp_admin_url('export.php'));
    }
    if ($kind === 'markdown' || $kind === 'full') {
        mp_export_markdown_from_database($zip);
        mp_export_add_directory($zip, mp_content_path('versions'), 'legacy-content/versions');
    }
    if ($kind === 'static' || $kind === 'full') {
        if (function_exists('mp_generate_static_site_export')) {
            $staticTargetRoot = mp_static_site_export_root('export-' . date('Ymd-His') . '-' . bin2hex(random_bytes(4)));
            try {
                mp_generate_static_site_export($staticTargetRoot);
                mp_export_add_directory($zip, $staticTargetRoot, 'static');
            } finally {
                if (is_dir($staticTargetRoot)) {
                    mp_delete_directory($staticTargetRoot);
                }
            }
        }
        mp_export_add_directory($zip, mp_public_path('assets'), 'static/assets');
        mp_export_add_directory($zip, mp_public_path('media'), 'static/media');
    }
    if ($kind === 'media' || $kind === 'full') {
        mp_export_add_directory($zip, mp_public_path('media'), 'media');
    }
    if ($kind === 'database' || $kind === 'full') {
        $zip->addFromString('database/bonumark.sql', mp_export_database_sql());
    }

...
## 0.3.14 - Back to Stream Root URL Hotfix

- Corrected `bms_stream_home_url()` so the canonical stream home is the install root, not `/stream/`.
- Repaired single-post **Back to stream** links so they return to the Bonumark Stream homepage across all installs.
- Updated the default theme fallback so stale `stream/` or `/stream/` back-link values resolve to the site root.
- Preserved the existing single-post `/stream/{slug}/` route behavior while stopping `/stream/` from being treated as the canonical stream archive destination.
- Added migration `0097_back_to_stream_root_url_hotfix.php` and updated package metadata, documentation, and release manifest.

## 0.3.13 - Back to Stream Route Repair Pass

- Added a core `bms_stream_home_url()` helper so stream archive/home links are generated canonically instead of falling back to relative `stream/` paths.
- Repaired the single-post **Back to stream** link data and default theme fallback so the link returns to the stream archive instead of resolving as a nested or duplicate stream slug.
- Added defensive route handling for stale relative links such as `/stream/stream/` and `/stream/{slug}/stream/` so they resolve back to the stream archive instead of rendering a missing-post page.
- Added migration `0096_back_to_stream_route_repair.php` and updated package metadata, documentation, and release manifest.
...
- Documented the internal `bms_` PHP function prefix as a private Bonumark Stream namespace.
...
- Updated the database seed helper to use `bms_version()` instead of hard-coded v0.2.0 settings.

...
<nav class="stream-pagination pagination-load-more" aria-label="Stream pagination">
  <?php if (!empty($data['has_older'])): ?>
    <div class="pagination-older"><a href="<?= htmlspecialchars((string)($data['older_url'] ?? '#'), ENT_QUOTES, 'UTF-8') ?>"><?= htmlspecialchars((string)($data['older_label'] ?? 'Load More'), ENT_QUOTES, 'UTF-8') ?></a></div>
  <?php else: ?>

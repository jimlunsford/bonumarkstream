...
function bms_stream_like_cookie_value(): string
{
    $cookieName = bms_stream_like_cookie_name();
...
function bms_stream_like_visitor_hash(): string
{
    $key = bms_stream_like_cookie_value();
    $salt = (string)(bms_config()['security_salt'] ?? 'bonumark-stream');
    return hash('sha256', $key . '|' . $salt);
}

function bms_stream_like_ip_hash(): string
{
...
function bms_stream_ensure_like_attempts_table(): void
{
    static $done = false;
    if ($done || !bms_is_installed() || !bms_has_database_config()) {
        return;
    }

    $sql = "CREATE TABLE IF NOT EXISTS `" . bms_table('stream_like_attempts') . "` (
...
function bms_stream_like_enforce_rate_limit(string $slug, string $visitorHash): void
{
    bms_stream_ensure_like_attempts_table();
    $ipHash = bms_stream_like_ip_hash();

    try {
        bms_db()->exec('DELETE FROM ' . bms_table('stream_like_attempts') . ' WHERE attempted_at < (NOW() - INTERVAL 1 DAY)');

        $stmt = bms_db()->prepare('SELECT COUNT(*) FROM ' . bms_table('stream_like_attempts') . ' WHERE (visitor_hash = :visitor_hash OR ip_hash = :ip_hash) AND attempted_at >= (NOW() - INTERVAL 60 SECOND)');
        ...
        $insert = bms_db()->prepare('INSERT INTO ' . bms_table('stream_like_attempts') . ' (post_slug, visitor_hash, ip_hash, attempted_at) VALUES (:post_slug, :visitor_hash, :ip_hash, NOW())');
        $insert->execute(['post_slug' => bms_slugify($slug), 'visitor_hash' => $visitorHash, 'ip_hash' => $ipHash]);
...
function bms_stream_like_label(int $count): string
{
    return number_format(max(0, $count)) . ' ' . ($count === 1 ? 'like' : 'likes');
}

function bms_stream_ensure_likes_table(): void
{
    static $done = false;
    if ($done || !bms_is_installed() || !bms_has_database_config()) {
        return;
    ...
    $sql = "CREATE TABLE IF NOT EXISTS `" . bms_table('stream_likes') . "` (
      `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
...
function bms_stream_post_record_by_slug(string $slug): ?array
{
    ...
    try {
        $stmt = bms_db()->prepare('SELECT id, slug, status, post_type FROM ' . bms_table('posts') . ' WHERE slug = :slug AND status = :status AND post_type = :post_type LIMIT 1');
...
function bms_stream_visitor_liked_slug(string $slug): bool
{
    ...
    try {
        bms_stream_ensure_likes_table();
        $stmt = bms_db()->prepare('SELECT id FROM ' . bms_table('stream_likes') . ' WHERE post_id = :post_id AND visitor_hash = :visitor_hash LIMIT 1');
        $stmt->execute([
            'post_id' => (int)$post['id'],
            'visitor_hash' => bms_stream_like_visitor_hash(),
        ]);
...
function bms_stream_like_status_for_slugs(array $slugs): array
{
    $cleanSlugs = [];
    ...
    if (!$cleanSlugs || !bms_is_installed() || !bms_has_database_config()) {
        return [];
    }
    ...
    try {
        ...
        $stmt = bms_db()->prepare('SELECT id, slug FROM ' . bms_table('posts') . ' WHERE status = :status AND post_type = :post_type AND slug IN (' . implode(',', $placeholders) . ')');
        $stmt->execute($params);
        $posts = [];
        $ids = [];
...
function bms_stream_register_like(string $slug): array
{
    $post = bms_stream_post_record_by_slug($slug);
    if (!$post) {
    ...
    $visitorHash = bms_stream_like_visitor_hash();
    bms_stream_like_enforce_rate_limit($slug, $visitorHash);

    try {
        bms_stream_ensure_likes_table();
        $stmt = bms_db()->prepare('INSERT IGNORE INTO ' . bms_table('stream_likes') . ' (post_id, post_slug, visitor_hash, created_at) VALUES (:post_id, :post_slug, :visitor_hash, NOW())');
        $stmt->execute([
            'post_id' => (int)$post['id'],
    ...
    $status = bms_stream_like_status_for_slugs([(string)$post['slug']]);
    return $status[(string)$post['slug']] ?? [
        'slug' => (string)$post['slug'],
        'liked' => true,
        'count' => bms_stream_like_count_for_slug((string)$post['slug']),
        'label' => bms_stream_like_label(bms_stream_like_count_for_slug((string)$post['slug'])),
    ];
}

...
function bms_public_profile_url_for_user(array $user): string
{
    ...
    if ($username !== '') {
        return bms_url_path('profile.php?user=' . rawurlencode(bms_normalize_username($username)));
    }
    ...
    return bms_url_path('profile.php');
}
...
function bms_find_public_user_by_id(int $id): ?array
{
    ...
    $stmt = bms_db()->prepare('SELECT id, username, display_name, email, role, status, bio, website, social_links, profile_visibility, avatar_path, created_at, updated_at FROM ' . bms_table('users') . ' WHERE id = :id AND status = :status LIMIT 1');
    $stmt->execute([
        'id' => $id,
        'status' => 'active',
    ]);
...
{
    bms_require_installed();
    $stmt = bms_db()->prepare('SELECT id, username, display_name, email, role, status, bio, website, social_links, profile_visibility, avatar_path, created_at, updated_at FROM ' . bms_table('users') . ' WHERE username = :username AND status = :status LIMIT 1');
    $stmt->execute([
...
function bms_find_public_user_by_handle(string $handle): ?array
{
    $user = bms_find_public_user_by_username($handle);
    if ($user) {
        return $user;
    }

    $wanted = bms_normalize_username($handle);
    if ($wanted === '') {
        return null;
    }

    try {
        $stmt = bms_db()->prepare('SELECT id, username, display_name, email, role, status, bio, website, social_links, profile_visibility, avatar_path, created_at, updated_at FROM ' . bms_table('users') . ' WHERE status = :status ORDER BY id ASC LIMIT 500');
        $stmt->execute(['status' => 'active']);
        foreach (($stmt->fetchAll() ?: []) as $candidate) {
            if (!is_array($candidate)) {
                continue;
            }
            $candidateUsername = bms_normalize_username((string)($candidate['username'] ?? ''));
            $candidateDisplay = bms_normalize_username((string)($candidate['display_name'] ?? ''));
            if ($wanted === $candidateUsername || $wanted === $candidateDisplay) {
                return bms_profile_user_is_viewable($candidate) ? $candidate : null;
            }
        }
    } catch (Throwable $e) {
        return null;
...
function bms_author_for_stream_page(array $page): array
{
    ...
    try {
        ...
        $stmt = bms_db()->prepare('SELECT u.id, u.username, u.display_name, u.email, u.role, u.status, u.bio, u.website, u.social_links, u.profile_visibility, u.avatar_path, u.created_at, u.updated_at FROM ' . bms_table('posts') . ' p INNER JOIN ' . bms_table('users') . ' u ON u.id = p.author_id WHERE p.slug = :slug AND p.status = :status AND u.status = :user_status LIMIT 1');
...
{
    try {
        $stmt = bms_db()->prepare('SELECT COUNT(*) FROM ' . bms_table('posts') . ' WHERE author_id = :author_id AND status = :status');
...
function bms_profile_recent_posts(int $userId, int $limit = 100): array
{
    ...
    try {
        $stmt = bms_db()->prepare('SELECT title, slug, description, published_at, updated_at, date_published FROM ' . bms_table('posts') . ' WHERE author_id = :author_id AND status = :status ORDER BY COALESCE(published_at, updated_at, created_at) DESC LIMIT ' . $limit);
        $stmt->execute(['author_id' => $userId, 'status' => 'published']);
...
function bms_profile_activity_excerpt(array $post, int $limit = 150): string
{
...
function bms_profile_activity_date_label(array $post): string
{
...
function bms_profile_page_html(?array $user): string
{
    ...
    $canonicalPath = $user ? bms_public_profile_url_for_user($user) : bms_url_path('profile.php');
    $recentPosts = [];

    if ($user) {
        foreach (bms_profile_recent_posts((int)$user['id'], 100) as $post) {
            $slug = bms_slugify((string)($post['slug'] ?? ''));
            $recentPosts[] = [
                'url' => $slug !== '' ? bms_stream_url($slug) : '',
                'label' => 'Open post',
                'excerpt' => bms_profile_activity_excerpt($post),
                'date_label' => bms_profile_activity_date_label($post),
                'raw' => $post,
            ];
        }
    }
    ...
    $view = [
        ...
        'profile_links' => $user ? bms_profile_social_links_for_user($user) : [],
        'avatar_markup' => $user ? bms_user_avatar_markup($user, '', 192, 192) : '',
        'post_count' => $user ? bms_profile_post_count((int)$user['id']) : 0,
        'comment_count' => $user ? bms_profile_comment_count((int)$user['id']) : 0,
        'member_since' => $user ? bms_profile_member_since_label($user) : '',
